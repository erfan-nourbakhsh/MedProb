...
def parse_args():
    p = argparse.ArgumentParser(
        description="Cross-dataset probing on per-layer embeddings"
    )
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Dataset name used to load train labels and train embeddings",
    )
    p.add_argument(
        "--test_dataset_embedding",
        type=str,
        required=True,
        help="Dataset name used to load test labels and test embeddings",
    )
    p.add_argument(
        "--train_dataset_dir",
        type=str,
        default=None,
        help="Path to the train dataset directory; defaults to ./samples/<train_dataset_embedding>",
    )
    p.add_argument(
        "--test_dataset_dir",
        type=str,
        default=None,
        help="Path to the test dataset directory; defaults to ./samples/<test_dataset_embedding>",
    )
    return p.parse_args()
...
def main():
    args = parse_args()
    train_dataset_dir = (
        args.train_dataset_dir or f"./samples/{args.train_dataset_embedding}"
    )
    test_dataset_dir = (
        args.test_dataset_dir or f"./samples/{args.test_dataset_embedding}"
    )
    c_values = (
    ...
    print("=" * 70)
    print(
        f"CROSS-DATASET PROBING — {args.probe_task.upper()} / {args.probe_type.upper()}"
    )
    print("=" * 70)
    ...
    print(f"Train Dataset:        {args.train_dataset_embedding}")
    print(f"Train Dataset Dir:    {train_dataset_dir}")
    print(f"Test Dataset:         {args.test_dataset_embedding}")
    print(f"Test Dataset Dir:     {test_dataset_dir}")
    print(f"Selectivity:          {bool(args.selectivity)}")
    print(f"CV folds:             {args.cv}")
    print("=" * 70)
    train_data, _ = load_dataset(train_dataset_dir, args.train_dataset_embedding)
    _, test_data = load_dataset(test_dataset_dir, args.test_dataset_embedding)
    if args.probe_task == "answer_decoding":
        ...
        if y_train is None or y_test is None:
            print("[ERROR] Prompting predictions not found for one or both datasets")
            sys.exit(1)
    ...
    train_embed_path = (
        f"./data/sample_features/{args.model_stem}/"
        f"{args.train_dataset_embedding}/{config_name}{position_suffix}"
    )
    test_embed_path = (
        f"./data/sample_features/{args.model_stem}/"
        f"{args.test_dataset_embedding}/{config_name}{position_suffix}"
    )
    ...
    task_dir = f"{args.probe_task}_{args.probe_type}_cross_dataset"
    transfer_name = (
        f"train_{args.train_dataset_embedding}__test_{args.test_dataset_embedding}"
    )
    out_root = (
        f"./data/sample_generations/{args.model_stem}/cross_dataset/"
        f"{task_dir}/{transfer_name}/{config_name}{position_suffix}"
    )
    ...
    for layer_key in tqdm(
        layer_keys, desc=f"Cross-dataset probing per layer ({args.probe_type})"
    ):
        ...
        for sample, pred in zip(test_data.samples, predictions):
            record = {
                ...
                "probe_type": args.probe_type,
                "train_dataset_embedding": args.train_dataset_embedding,
                "test_dataset_embedding": args.test_dataset_embedding,
            }
