...
for dataset in "${DATASETS[@]}"; do
    print_dataset_skip_summary "./samples/${dataset}"
done
                            ...
                            for probe_task in "${PROBE_TASKS[@]}"; do
                                for probe_type in "${PROBE_TYPES[@]}"; do
                                    for position in "${POSITIONS[@]}"; do
                                        train_config="${approach}_${reasoning}_${options_mode}"
                                        test_config="${approach}_${reasoning}_${options_mode}_${test_options_order}"
                                        ...
                                        task_dir="${probe_task}_${probe_type}_default_train"
                                        transfer_name="train_${train_config}__test_${test_config}"
                                        run_dir="${ROOT}/${model}/${dataset}/${task_dir}/${transfer_name}${pos_suffix}"
                                        desc="dataset=${dataset} | model=${model} | train=${TRAIN_OPTIONS_ORDER} | test=${test_options_order} | ${task_dir} | pos=${position}"
...
echo "=================================================================="
echo "[DONE] Default-train probing evaluation complete"
